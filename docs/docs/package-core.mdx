...
<PackageHeader version="1.0.1" packageName="core" containsTypeDefs />
...
A simple setup with BlueLibs Core and a simple bundle:

- https://stackblitz.com/edit/typescript-mhek88?file=index.ts
...
There are two ways to inject, via constructor, or as a property:
...
container.set({ id: MY_SERVICE_TOKEN, type: Emptyness }); // A simple B class
container.get(MY_SERVICE_TOKEN); // this is a singleton B instance

// You have two ways of injecting things in a class:
class A {
...
Our solution to this is to use `dotenv` and craft our own `env.ts` file which exports a type-safe constant:
...
Test your knowledge, see if everything is clear and see how many points you get!
