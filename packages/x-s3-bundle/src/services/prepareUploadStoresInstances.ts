...
import { AzureStorageService } from "./StoreServices/AzureStorageService";

export const prepareUploadStoresInstances = (
  config: XS3BundleConfigType,
  container: any
) => {
  config.stores = config.stores.map((store: IStoreUploadService) => {
    switch (true) {
      case store instanceof S3Service: {
        if (!(store.credentials as AWSS3Config))
          throw "wrong credentials type for s3 store";
        break;
      }
      case store instanceof AzureStorageService: {
        if (!(store.credentials as AzureStorageConfig))
          throw "wrong credentials type for azure store";
        break;
      }
      case store instanceof LocalService: {
        ...
        credentials.downloadUrl =
          container.get(ApolloBundle).getConfig().url + credentials.downloadUrl;
            ...
            : async (container, req, res) => {
                ...
                await mkdirp(dirname(key), async function (err) {
                  if (err) {
                    ...
                    fs.writeFile(key, Buffer.from(buffer, "base64"), (err) => {
                      if (err) {
                        throw "something went wrong with creating the local file";
                      }
                      const file = key;
