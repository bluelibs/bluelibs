      ...
      render: (props) => {
        ...
        <Ant.Form.Item {...props}>
          <Ant.Select
            ...
            placeholder={t('{{ title }}')}
          >
          {options.map((option)=>(<Ant.Select.Option value={option.value} key={option.value} >{t(`{{ title }}_enums.${option.label.toLowerCase()}`)?t(`{{ title }}_enums.${option.label.toLowerCase()}`): option.label}</Ant.Select.Option>))}
          </Ant.Select>
        </Ant.Form.Item>
      )},
    {{/ if }}
